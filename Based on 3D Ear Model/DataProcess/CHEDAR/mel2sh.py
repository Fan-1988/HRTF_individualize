...
def calculate_sh_coefficients(sofa_file_path, max_order):
    # 从SOFA文件中读取HRIR数据
    directions, _ = read_sofa_hrir("/data/fyw/dataset/dataset/chedar/hrtfs/chedar_0006_UV1m.sofa")
    hrtf_data = load_npy_data(sofa_file_path)
    print(hrtf_data.shape)
    hrtf_data = hrtf_data[:, 0, 20:56]  # 假设只处理一个耳朵
...
if __name__ == "__main__":
    ...
    mel_npy_folder = "/data/fyw/dataset/dataset/chedar/hrtfs_mel64_log/"
    target_folder = "/data/fyw/dataset/dataset/chedar/hrtfs_mel64_log_sh/"
    # 确保目标文件夹存在
    if not os.path.exists(target_folder):
        os.makedirs(target_folder)
    # 使用glob找到所有满足模式的SOFA文件
    sofa_files = glob.glob(os.path.join(mel_npy_folder, "chedar_*_UV1m.npy"))
