...
class BasicBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        out = self.bn1(out)
        print("first bn", x.shape)
        out = self.relu(out)
        print("first relu", x.shape)

        out = self.conv2(out)
        print("next conv", x.shape)
        out = self.bn2(out)
        print("next bn", x.shape)

        if self.downsample is not None:
            residual = self.downsample(x)

        out += residual
        print("add residual", out.shape)
        out = self.relu(out)
        print("last relu", out.shape)
        return out
...
def test():
    ...
    input = np.random.rand(30,1,32,32,32)
    input_tensor = torch.from_numpy(input)

    # Convert tensor to floating point
    input = input_tensor.float()  # Now it's a float tensor
    model.forward(input)
