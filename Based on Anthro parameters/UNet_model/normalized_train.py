...
from normalized_unet_model import UNet as normalized_UNet
from normalized_dataset import create_combined_dataset

# 初始化日志记录器
def setup_logger(log_prefix="mag_normalized_training_log", log_dir="logs"):
    # 获取当前时间作为日志文件名的一部分
...
def main():
    ...
    log_filename = setup_logger("mag_normalized_training_log_5fold",log_dir="/data/zym/hrtf_prediction/2023_caai/log")
    ...
    dataset = create_combined_dataset(hutubs_path="/data/zym/dataset/normalized/hutubs_normalized_hrtf_129.npy",
                                      cipic_path="/data/zym/dataset/normalized/cipi_normalized_hrtf_129.npy")
    ...
    torch.save(model.state_dict(), '/data/zym/hrtf_prediction/2023_caai/mag_model/5fold_normalized_hrtf_model.pth')
...
def RMSE_LSD():
    ...
    model.load_state_dict(torch.load('/data/zym/hrtf_prediction/2023_caai/mag_model/0.2_normalized_hrtf_model.pth'))
    model.to(device)
    model.eval()  # 设置为评估模式
